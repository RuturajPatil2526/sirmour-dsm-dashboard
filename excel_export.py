...
def build_excel_report(block_df: pd.DataFrame, summary: dict, plant: PlantConfig, figures: dict = None) -> bytes:
    ...
    if has_enercast:
        e_mask = block_df["Enercast_MW"].notna()
        e_n = int(e_mask.sum())
    ...
    for offset in range(n):
        ...
        if has_enercast:
            erow = block_df.iloc[offset]
            if pd.notna(erow.get("Enercast_MW")):
                ws.write_formula(r0, 9, f"=D{r}-I{r}", mw_fmt, float(erow["Enercast_Deviation_MW"]))
                ws.write_formula(r0, 10, f"=J{r}/$C${cap_row}*100", pct_fmt, float(erow["Enercast_Deviation_%"]))
                terms_e = []
                prev_bound = "0"
                for slab, srow in zip(plant.dsm_slabs, slab_rows):
                    rate_ref = f"$D${srow}"
                ...
                ws.write_formula(r0, 11, e_formula, pen_fmt, float(erow["Enercast_Penalty"]))
            else:
                for col in (9, 10, 11):
    ...
    if n:
        ...
        # Enercast adds 4 extra columns (I-L) to the main sheet -- shift the
        # chart anchors right so they never sit on top of that data.
        chart_offset = 4 if has_enercast else 0
